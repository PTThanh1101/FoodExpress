...
namespace FoodExpress.Admin
{
    public partial class OrderStatus : System.Web.UI.Page
    {
        ...
        private void getOrderStatus()
        {
            ...
            dt = new DataTable();
            sda.Fill(dt);
            reOrderStatus.DataSource = dt;
            reOrderStatus.DataBind();
        }

        protected void reOrderStatus_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            lblMsg.Visible = false;
            if (e.CommandName == "edit")
            {
                ...
                cmd.Parameters.AddWithValue("@PaymentId", e.CommandArgument);
                cmd.CommandType = CommandType.StoredProcedure;
                sda = new SqlDataAdapter(cmd);
                dt = new DataTable();
                sda.Fill(dt);
                ...
                hdnId.Value = e.CommandArgument.ToString(); // Lưu PaymentId vào HiddenField
        ...
        {
            int paymentId = Convert.ToInt32(hdnId.Value);
            con = new SqlConnection(Connection.GetConnectionString());
            cmd = new SqlCommand("Invoice", con);
